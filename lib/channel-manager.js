...
module.exports = function(server, log, config) {
    ...
    ChannelManager.prototype.openChannel = function(key, options) {
        ...
        if(!channel) {
            ...
            // Look for any pending subscriptions
            var pendingSubscriptions = this.pendingSubscriptions[key];
            if(pendingSubscriptions) {
                log.debug("Detected pending %d subscriptions for channel %s. They will all be subscribed", pendingSubscriptions.length, key);
                _.each(pendingSubscriptions, function(sub) {
                    channel.subscribe(sub.client);
                });

                // Remove processed pending subscriptions
    ...
    ChannelManager.prototype.subscribe = function(channelKey, client, options) {
        options = options || {};
        ...
        if(channel) {
            ...
            // Register a pending subscription. Will be applied when the channel is registered
            var pendingChannel = this.pendingSubscriptions[channelkey];
            if(!pendingChannel) {
                pendingChannel = this.pendingSubscriptions[channelKey] = [];
            }
    ...
    ChannelManager.prototype.discover = P.method(function() {
        ...
        return requireDirectory(module, Path.resolve(path), {
            visit: function(factory, filename) {
                ...
                _this.subscribe(event.channel, event.handler, event.options);
            },
            include: /event\.js$/
        });
